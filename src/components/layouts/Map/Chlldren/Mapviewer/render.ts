import { GfxContext, Texture, VertexBuffer, IndexBuffer, mat3, mat3mul, mat3mulx, mat3muly, mat3identity, mat3ortho } from "./gfx";
...
const OBJECTS_ATLAS: ObjectsAtlas = {
    ...
    sprites: OBJECTS_ATLAS_RAW.sprites.map((s) => ({
        ...s,
        uv: [
            { x: s.x / OBJECTS_ATLAS_RAW.width, y: s.y / OBJECTS_ATLAS_RAW.height },
            { x: (s.x + s.width) / OBJECTS_ATLAS_RAW.width, y: (s.y + s.height) / OBJECTS_ATLAS_RAW.height },
...
export class MapRenderer {
    ...
    config(name: keyof RendererConfig, value: RendererConfig[keyof RendererConfig]): void;
    config(name?: keyof RendererConfig, value?: RendererConfig[keyof RendererConfig]): RendererConfig | void {
        if (name === undefined) return this.cfg;

        (this.cfg as unknown as Record<string, unknown>)[name] = value;

        if (name === "background" || name === "polygons" || name === "scenery_back" ||
            name === "scenery_middle" || name === "scenery_front" || name === "highlight" ||
            name === "colliders" || name === "wireframe") {
            ...
            this.activeObjects = this.map.spawnpoints.filter((s) => list.includes(s.team));
        }
    }
    ...
    draw(x: number, y: number, s: number): void {
        ...
        const { gfx, objectsTexture, objectsVbo, objectsIbo, activeObjects } = this;
        if (this.cfg.objects && activeObjects.length > 0) {
            ...
            gfx.draw(gfx.Triangles, objectsVbo, objectsIbo, 0, 6 * activeObjects.length);
        }
    }
    ...
    private drawBatch(batch: Batch): void {
        ...
        for (const call of batch.calls) {
            ...
            gfx.draw(batch.mode, vbo, ibo, batch.ibo_index + call.offset, call.count);
        }
    }
    ...
    private createTextureFromImage(image: HTMLImageElement, pot: boolean): Texture {
        const canvas = document.createElement("canvas");
        const ctx = canvas.getContext("2d")!;
        let w = image.width;
        let h = image.height;
        if (pot) {
            let pw = 1; while (pw < w) pw <<= 1; w = pw;
            let ph = 1; while (ph < h) ph <<= 1; h = ph;
        }
        canvas.width = w;
        canvas.height = h;
    ...
    private setupTextures(): void {
        ...
        this.blackTexture = gfx.create_texture(1, 1, gfx.RGBA, (_x, _y, rgba) => {
            rgba[0] = rgba[1] = rgba[2] = 0.5; rgba[3] = 1;
        });

        this.colliderTexture = gfx.create_texture(256, 256, gfx.RGBA, (x, y, rgba) => {
            const dx = x - 128, dy = y - 128;
            const dist = Math.sqrt(dx * dx + dy * dy);
            const inner = 1, outer = 0.5;
        ...
        this.colliderTexture.filter(gfx.LinearMipmapLinear, gfx.Linear ?? gfx.Nearest);

        this.objectsTexture = gfx.White;

        for (let i = 0; i < map.images.length + 1; i++) this.textures.push(gfx.White);
        this.textures.unshift(this.objectsTexture, this.colliderTexture, this.blackTexture, gfx.White);
    }
    ...
    private loadImages(): void {
        ...
        for (let i = 0; i < map.objects.length; i++) {
            texturePaths[map.objects[i]!.style] = this.imagePath("scenery-gfx/" + map.images[map.objects[i]!.style - 1]!);
        }
    ...
    private getEdges(): [[{ x: number; y: number; u: number; v: number; color: readonly number[] }, { x: number; y: number; u: number; v: number; color: readonly number[] }]] {
        const polys = [...this.map.polygons, ...this.map.bgpolygons];
        type Edge = [
            { x: number; y: number; u: number; v: number; color: readonly number[] },
            { x: number; y: number; u: number; v: number; color: readonly number[] },
        ];
        const list: Edge[] = [];
        ...
        const isVertEq = (a: { x: number; y: number }, b: { x: number; y: number }) =>
        ...
        filtered.reverse();
        return filtered as unknown as [[{ x: number; y: number; u: number; v: number; color: readonly number[] }, { x: number; y: number; u: number; v: number; color: readonly number[] }]];
    }

    private setupMatrix(m: Float32Array, obj: { x: number; y: number; rotation: number; scalex: number; scaley: number }): Float32Array {
        const c = Math.cos(obj.rotation), s = Math.sin(obj.rotation);
        m[0] = c * obj.scalex; m[3] = -s * obj.scaley; m[6] = obj.x;
        m[1] = s * obj.scalex; m[4] =  c * obj.scaley; m[7] = -obj.y;
        m[2] = 0; m[5] = 0; m[8] = 1;
        return m;
    }
    ...
    private init(): void {
        ...
        const edges = this.getEdges() as unknown as Array<[{ x: number; y: number; u: number; v: number; color: readonly number[] }, { x: number; y: number; u: number; v: number; color: readonly number[] }]>;

        const vboSize = 4 + 3 * bgpolygons.length +
            4 * sceneryBack.length + 4 * sceneryMiddle.length + 4 * sceneryFront.length +
            3 * polygons.length + 3 * (polygons.length + bgpolygons.length) +
            2 * edges.length + 4 * colliders.length;

        const iboSize = 6 + 3 * bgpolygons.length +
            6 * sceneryBack.length + 6 * sceneryMiddle.length + 6 * sceneryFront.length +
            3 * polygons.length + 3 * (polygons.length + bgpolygons.length) +
            2 * edges.length + 6 * colliders.length;
        ...
        const vbo = this.vbo, ibo = this.ibo;
        const idx = { vbo: 0, ibo: 0 };

        // Background
        const bgBatch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        bgBatch.calls.push({ offset: idx.ibo, count: 6, texture: -1 });
        const d = map.sector_division, n = map.num_sectors;
        ibo.set(idx.ibo++, idx.vbo + 0); ibo.set(idx.ibo++, idx.vbo + 1); ibo.set(idx.ibo++, idx.vbo + 2);
        ibo.set(idx.ibo++, idx.vbo + 2); ibo.set(idx.ibo++, idx.vbo + 3); ibo.set(idx.ibo++, idx.vbo + 0);
        ...
        // BG polygons
        const bgPolyBatch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        bgPolyBatch.calls.push({ offset: 0, count: 3 * bgpolygons.length, texture: 0 });
        for (const tri of bgpolygons) {
            ibo.set(idx.ibo++, idx.vbo); ibo.set(idx.ibo++, idx.vbo + 1); ibo.set(idx.ibo++, idx.vbo + 2);
            for (const v of tri.vertices) vbo.set(idx.vbo++, v.x, -v.y, v.u, v.v, v.color);
        }
        this.batches.bgpolygons = bgPolyBatch;

        // Scenery batches
        this.batches.scenery_back = this.createSceneryBatch(sceneryBack, idx);
        this.batches.scenery_middle = this.createSceneryBatch(sceneryMiddle, idx);
        this.batches.scenery_front = this.createSceneryBatch(sceneryFront, idx);

        // Polygons
        const polyBatch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        polyBatch.calls.push({ offset: 0, count: 3 * polygons.length, texture: 0 });
        ...
        const hlBatch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        hlBatch.calls.push({ offset: 0, count: 0, texture: -1 });
        const allPolys = [...polygons, ...bgpolygons];
        for (const tri of allPolys) {
            ibo.set(idx.ibo++, idx.vbo); ibo.set(idx.ibo++, idx.vbo + 1); ibo.set(idx.ibo++, idx.vbo + 2);
            for (const v of tri.vertices) vbo.set(idx.vbo++, v.x, -v.y, 0, 0, hlColor);
        }
        this.batches.highlight = hlBatch;

        // Wireframe
        const wireBatch: Batch = { mode: gfx.Lines, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        wireBatch.calls.push({ offset: 0, count: 2 * edges.length, texture: -1 });
        for (const [e0, e1] of edges) {
            ibo.set(idx.ibo++, idx.vbo); ibo.set(idx.ibo++, idx.vbo + 1);
            vbo.set(idx.vbo++, e0.x, -e0.y, e0.u, e0.v, [e0.color[0]!, e0.color[1]!, e0.color[2]!, 255]);
            vbo.set(idx.vbo++, e1.x, -e1.y, e1.u, e1.v, [e1.color[0]!, e1.color[1]!, e1.color[2]!, 255]);
        }
        this.batches.wireframe = wireBatch;

        // Colliders
        const colliderColor: readonly number[] = [255, 0, 0, 255];
        const colBatch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        colBatch.calls.push({ offset: 0, count: 6 * colliders.length, texture: -3 });
        for (const col of colliders) {
            const cx = col.x, cy = -col.y, r = col.radius / 2.0;
            ibo.set(idx.ibo++, idx.vbo); ibo.set(idx.ibo++, idx.vbo + 1); ibo.set(idx.ibo++, idx.vbo + 2);
            ibo.set(idx.ibo++, idx.vbo + 2); ibo.set(idx.ibo++, idx.vbo + 3); ibo.set(idx.ibo++, idx.vbo);
        ...
        this.objectsVbo = gfx.create_vbo(4 * map.spawnpoints.length, gfx.Stream);
        this.objectsIbo = gfx.create_ibo(6 * map.spawnpoints.length, gfx.Stream);
    ...
    private createSceneryBatch(scenery: PmsMap["objects"], idx: { vbo: number; ibo: number }): Batch {
        const { gfx, vbo, ibo } = this;
        const batch: Batch = { mode: gfx.Triangles, ibo_index: idx.ibo, vbo_index: idx.vbo, calls: [] };
        if (scenery.length === 0) return batch;

        let call: DrawCall = { offset: 0, count: 0, texture: scenery[0]!.style };
        const m = mat3();

        for (const obj of scenery) {
            if (obj.style !== call.texture) {
                batch.calls.push(call);
                call = { offset: call.offset + call.count, count: 0, texture: obj.style };
            }
            this.setupMatrix(m, obj);
            const w = obj.width, h = obj.height, color = obj.color;
            ibo.set(idx.ibo++, idx.vbo); ibo.set(idx.ibo++, idx.vbo + 1); ibo.set(idx.ibo++, idx.vbo + 2);
            ibo.set(idx.ibo++, idx.vbo + 2); ibo.set(idx.ibo++, idx.vbo + 3); ibo.set(idx.ibo++, idx.vbo);
    ...
    private updateActiveBatches(): void {
        ...
        if (cfg.scenery_middle) this.activeBatches.push(batches.scenery_middle!);
    ...
    private updateHighlightBatch(): void {
        ...
        const matching = allPolys.map((_, i) => i).filter((i) => types.includes(allPolys[i]!.type));
