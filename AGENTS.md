...
- `src/api/`: one file per resource (`maps.ts`, `events.ts`, `users.ts`, `clans.ts`, `countries.ts`, `stats.ts`), each wrapping the shared `apiClient` (`src/api/client.ts`) with typed params and response shapes. `schema.d.ts` is auto-generated by `npm run generate:api-types` from the backend's OpenAPI schema; don't edit it by hand.
