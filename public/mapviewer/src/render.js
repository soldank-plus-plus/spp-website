...
function Renderer(gfx, map, root, on_ready)
{
	...
	Batch.prototype.draw = function()
	{
		...
		{
			var call = calls[i];
			gfx.bind(textures[call.texture + 4] || gfx.White); // "+ x" -> amount unshifted after loading textures
	...
	function get_edges()
	{
		var polys = map.polygons.concat(map.bgpolygons);
		var list = [];
		var filtered = [];
	...
	function init()
	{
		...
		var bgpolygons = map.bgpolygons;
		var highlight = map.polygons.concat(map.bgpolygons);
		var edges = get_edges();
		...
		// bgpolygons batch
		batches.bgpolygons = new Batch(gfx.Triangles, idx.ibo, idx.vbo);
		batches.bgpolygons.calls.push(new DrawCall(0, 3 * bgpolygons.length, 0));
		
		for (var i = 0, n = bgpolygons.length; i < n; i++)
		{
			var triangle = bgpolygons[i];
			ibo.set(idx.ibo++, idx.vbo + 0);
			ibo.set(idx.ibo++, idx.vbo + 1);
			ibo.set(idx.ibo++, idx.vbo + 2);

			// If (triangle.type>=24) ibo.set(0,0);
			for (var j = 0; j < 3; j++)
			{
				// If (triangle.type>=24) break;
		...
		var concatpolygons = polygons.concat(bgpolygons);
		
		for (var i = 0, n = concatpolygons.length; i < n; i++)
		{
			var triangle = concatpolygons[i];
	...
	function update_active_batches()
	{
		...
		config.polygons       && active_batches.push(batches.bgpolygons);
	...
	function update_highlight_batch()
	{
		...
		var polygons = map.polygons.concat(map.bgpolygons);
	...
	load_textures(function() {
		if (textures[0]) textures[0].wrap(gfx.Repeat, gfx.Repeat);
